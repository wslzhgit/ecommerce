layui.use(['table','form','laydate','layer','jquery'], function() {
    ...
    form.on('submit(demo2)', function(data){
        ...
        saveJosnWebUser['userheader'] = 'http://q6jzflwi2.bkt.clouddn.com/7.gif';
    ...
    //执行用户添加
    function saveWebUser(saveJosnWebUser) {
        $.ajax({
            type:"POST",  //请求方式，POST请求
            url:"webusers/saveT",   //访问服务器端的路径
            data:saveJosnWebUser,
            success:function (data) {  //请求执行正常函数回调
                var  msg="";
                if(data=='saveSuccess'){
                ...
                sendEmail(saveJosnWebUser.email.msg);
            },
            error:function () {  //请求执行异常时的函数回调
                layer.msg('服务器异常', {icon: 3,time:2000,anim: 6,shade:0.5});
            }
